...
_GIVEAWAY_KW = (
    "giveaway", "give away", "giving away", "win", "winner", "contest",
    "raffle", "prize", "draw", "sweepstake", "enter to win", "free gpu",
    "free access", "sponsored", "sponsor", "entries", "entry", "bonus entries",
    "抽奖", "送", "赠送", "赢", "免费", "中奖", "参与",
)
_COMMERCE_KW = (
...
def _detect_content_type(text: str) -> str:
    t = text.lower()
    if any(k in t for k in _GIVEAWAY_KW):   # before commerce — giveaways contain "$"
        return "giveaway"
...
def _why_it_worked(text: str, stats: dict[str, Any], content_type: str) -> list[str]:
    ...
    if content_type == "giveaway":
        numbers = _extract_numbers(text)
        if numbers:
            reasons.append(
                f"High-value prize ({', '.join(numbers[:3])}) creates immediate FOMO — "
                "readers share it because they want their network to see it too."
            )
        reasons.append(
            "FAQ format removes every excuse not to enter — "
            "each objection ('is it free?', 'where do I enter?') is answered before it's asked."
        )
        if views and likes:
            reasons.append(
                "Giveaway posts are inherently viral: every entrant has incentive to share "
                "their referral link, turning participants into promoters."
            )
...
_STEPS_BY_TYPE: dict[str, list[str]] = {
    "giveaway": [
        "Check the deadline — if it's within 2 weeks, enter now before you forget.",
        "Complete the minimum entry requirements (registration, follow, etc.).",
        "Collect every bonus entry available — referrals and social follows compound fast.",
        "Set a calendar reminder for the proof-of-attendance or submission deadline.",
        "Share your referral link once — one post is enough, spamming hurts credibility.",
    ],
    "commerce": [
...
def _keyword_hooks(text: str, content_type: str) -> list[str]:
    ...
    # Giveaway always gets its own hooks regardless of keyword matches
    if content_type == "giveaway":
        hooks = [
            "Enter now — giveaway posts decay fast; most entries arrive in the first 24h.",
            "Collect every bonus entry (referrals, follows, signups) before sharing — maximise your own count first.",
            "Verify sponsorship on the brand's official account before investing time.",
            "Set a calendar reminder for the proof/submission deadline — missing it disqualifies you.",
        ]
    ...
    if not hooks:
        defaults = {
            ...
            "giveaway": ["Enter immediately, collect all bonus entries, set a deadline reminder."],
        }
